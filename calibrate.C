...
void calibrate(){
	...
	if( plot_dxdy ){
		...
		h_dxdy_all = new TH2D("h_dxdy_all",Form("HCal dxdy (NO CUTS) - SBS = %i%%, %s, %s;y_{HCAL}-y_{expect} (m); x_{HCAL}-x_{expect} (m)", sbsfieldscale, run_target.Data(), runs_string.Data()),125,-2,2,125,-4,6);
		h_dx = new TH1D( "h_dx", Form("HCal dx - SBS = %i%%, %s, %s; m", sbsfieldscale, run_target.Data(), runs_string.Data()), 200, -4.0, 2.0 );
		h_dy = new TH1D( "h_dy", Form("HCal dy - SBS = %i%%, %s, %s; m", sbsfieldscale, run_target.Data(), runs_string.Data()), 100, -1.2, 1.2 );
		h_Y = new TH1D( "h_Y", Form("HCal Y - SBS = %i%%, %s, %s; m", sbsfieldscale, run_target.Data(), runs_string.Data()), 100, -1.2, 1.2 );
	}
