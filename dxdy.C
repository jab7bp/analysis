...
void dxdy(){
	...
	if( crosstalk ){
		outfile = new TFile(Form("rootfiles/%i_%s_dxdy.root", runnum, XTALK_ONOFF.Data()), "RECREATE");		
	}
	...
	h_dx = new TH1D("h_dx",Form("dx (NO CUTS) - SBS = %i%%, %s, Run %i; x_{HCal} - x_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -2.5,2.5);
	h_dx_cut = new TH1D("h_dx_cut",Form("dx (Basic CUTS) - SBS = %i%%, %s, Run %i; x_{HCal} - x_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -2.5,2.5);
	h_dx_wcut = new TH1D("h_dx_wcut",Form("dx (W cut) - SBS = %i%%, %s, Run %i; x_{HCal} - x_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -2.5,2.5);
	h_dx_fcut = new TH1D("h_dx_fcut",Form("dx (f cut) - SBS = %i%%, %s, Run %i; x_{HCal} - x_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -2.5,2.5);
	h_dy = new TH1D("h_dy",Form("dy (NO CUTS) - SBS = %i%%, %s, Run %i; y_{HCal} - y_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -1.25,1.25);
	h_dy_cut = new TH1D("h_dy_cut",Form("dy (Basic Cuts) - SBS = %i%%, %s, Run %i; y_{HCal} - y_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -1.25,1.25);  
	h_dy_wcut = new TH1D("h_dy_wcut",Form("dy (W Cuts) - SBS = %i%%, %s, Run %i; y_{HCal} - y_{exp} (m);", sbsfieldscale, run_target.Data(), runnum), 250, -1.25,1.25);  
	...
	W_mean = lookup_cut(runnum, "W");
	W_sigma = lookup_cut(runnum, "W_sigma");
...
//FIDUCIAL CUT:
	...
	master_cut_vec = {
			...
			//11449:
			Form("bb.ps.e>%f", 0.15),
			Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))>(%f)&&((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))<(%f)", 0.9, 1.1),
...
//end of events loop  
	...
	cout << "Shower + Pre-Shower cut: "<< lookup_cut(runnum, "SH_PS_clus_e_cut") - lookup_cut(runnum, "SH_PS_sigma") << endl;
	cout << "Vertex <= 0.075" << endl;
	cout << "Number of GEM planes hit > 3" << endl;
	cout << "Number of tracks per event = 1" << endl;
	cout << "Pre-shower cut: " << lookup_cut(runnum, "PS_clus_e_cut") << endl;
	cout << "E/p: center = " << lookup_cut(runnum, "Ep") << ", sigma = " << lookup_cut(runnum, "Ep_sigma") << endl;
	cout << "HCal cluster energy: " << lookup_cut(runnum, "HCal_clus_e_cut");
	cout << "------------------------------------------------------------------"<< endl;
	cout << "Cut lookups: " << endl;
	cout << "PS: " << lookup_cut(runnum, "PS_clus_e_cut") << endl;
	cout << "SH_PS: " << lookup_cut(runnum, "SH_PS_clus_e_cut") << "; " << lookup_cut(runnum, "SH_PS_sigma") << endl;
	cout << "HCal_clus_e: " << lookup_cut(runnum, "HCal_clus_e_cut") << endl;
	cout << "Ep: " << lookup_cut(runnum, "Ep") << "; " << lookup_cut(runnum, "Ep_sigma") << endl;
	cout << "W2: " << lookup_cut(runnum, "W2") << "; " << lookup_cut(runnum, "W2_sigma") << endl;
	cout << "W: " << lookup_cut(runnum, "W") << "; " << lookup_cut(runnum, "W_sigma") << endl;
	cout << "------------------------------------------------------------------"<< endl << endl;
