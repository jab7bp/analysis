...
void dxdy_parsed_files(){
	...
	if( !crosstalk ){
		outfile = new TFile(Form("rootfiles/%s_SBS%i_mag%i_dxdy_parsed.root", run_target.Data(), kine, sbsfieldscale), "RECREATE");		
	}
	if( crosstalk ){
		outfile = new TFile(Form("rootfiles/%s_SBS%i_mag%i_%s_dxdy_parsed.root", run_target.Data(), kine, sbsfieldscale, XTALK_ONOFF.Data()), "RECREATE");		
	}
