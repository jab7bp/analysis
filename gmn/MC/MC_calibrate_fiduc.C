...
void MC_calibrate_fiduc(){
	...
	if( plot_dxdy ){
		h_dxdy_wcut = new TH2D("h_dxdy_wcut",Form("HCal dxdy (W cuts) - SBS = %i%%, %s, %s;y_{HCAL}-y_{expect} (m); x_{HCAL}-x_{expect} (m)", sbsfieldscale, run_target.Data(), runs_string.Data()), 600, -3, 3, 600, -3.0, 3.0 );
		h_dxdy_all = new TH2D("h_dxdy_all",Form("HCal dxdy (NO CUTS) - SBS = %i%%, %s, %s;y_{HCAL}-y_{expect} (m); x_{HCAL}-x_{expect} (m)", sbsfieldscale, run_target.Data(), runs_string.Data()),600,-3,3,600,-3,3);
	...
	HCal_theta = lookup_HCal_angle_by_kine(kine);		//Theta angle for HCal from downstream beamline
	cout << endl << "-----------------------------------" << endl;
