...
double BB_dist, BB_theta, BB_theta_rad, W_mean, W_sigma;
...
void MC_dxdy_old(){
	...
	if( !crosstalk ){
		outfile = new TFile(Form("rootfiles/MC_SBS%i_mag%i_dxdy.root", kine, sbsfieldscale), "RECREATE");		
	}
	if( crosstalk ){
		if( multi_run ){
			outfile = new TFile(Form("rootfiles/MC_SBS%i_mag%i_dxdy.root", kine, sbsfieldscale), "RECREATE");		
	...
	h_W = new TH1D("h_W", Form("Invariant Mass W - SBS%i = %i%%, %s; GeV", kine, sbsfieldscale, run_target.Data()), 300, 0.0, 3.0);h_W_cut = new TH1D("h_W_cut", Form("Invariant Mass W (Coin & Vert Cuts) - SBS%i = %i%%, %s; GeV", kine, sbsfieldscale, run_target.Data()), 300, 0.0, 3.0);
	...
	h_dx = new TH1D("h_dx",Form("dx (NO CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 250, -2.5,2.5);
	...
	h_dy = new TH1D("h_dy",Form("dy (NO CUTS) - SBS%i = %i%%, %s; y_{HCal} - y_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 850, -7.25,1.25);
	...
	h_dxdy = new TH2D("h_dxdy", Form("Hadron Spot(s) on HCal (NO CUTS) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 900, -7.5, 1.5, 500, -2.5, 2.5 );
	...
	h_dxdy_fcut = new TH2D("h_dxdy_fcut", Form("Hadron Spot(s) on HCal (f cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 850, -7.25, 1.25, 250, -2.5, 2.5 );
	...
	BB_theta = lookup_BB_angle_by_kine(kine);
	BB_theta_rad = BB_theta*(pi/180.0);

	HCal_dist = lookup_HCal_dist_by_kine(kine);
	HCal_theta = lookup_HCal_angle_by_kine(kine);
	HCal_theta_rad = HCal_theta*(pi/180.0);
	W_mean = lookup_MC_cut(kine, sbsfieldscale, "W");
	W_sigma = lookup_MC_cut(kine, sbsfieldscale, "W_sigma");

	dx_p = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dx_p");
	dx_p_sigma = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dx_p_sigma");
	dy_p = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dy");
	dy_p_sigma = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dy_sigma");
	dx_n = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dx_n");
	dx_n_sigma = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dx_n_sigma");
	dy_n = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dy");
	dy_n_sigma = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dy_sigma");
	dx_pn_max = lookup_dxdy_by_kine_and_mag(kine, sbsfieldscale, "dx_pn_max");
	...
	cout << "-----------------------------------" << endl;
	cout << "BB angle: " << BB_theta << endl;
	cout << "SBS angle: " << lookup_SBS_angle_by_kine(kine) << endl;
	...
	if( single_run ){
		cout << "Running in single run mode: SBS" << kine << ", Mag: " << sbsfieldscale << endl;
		cout << "--------------------------------------" << endl;
...
// Set BRANCH ADDRESSES
	...
	p_Beam = E_beam/(1.0 + E_beam/Mp*(1.0 - cos(BB_theta_rad)));

	E_loss_outgoing = cell_diameter/2.0/sin(BB_theta_rad)*rho_tgt*dEdx_tgt; //Should be about 1 MeV
...
//FIDUCIAL CUT:
	...
	master_cut_vec = {
			...
			// Form("bb.ps.e>%f", lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "PS_clus_e_cut")),
			// Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))>(%f)&&((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))<(%f)", lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "Ep") - (lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "Ep_sigma")), lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "Ep") + (lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "Ep_sigma"))),
			// // Form("((abs(((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))))-%f)<%f", lookup_cut(runnum, "Ep"), lookup_cut(runnum, "Ep_sigma")),
			// Form("sbs.hcal.e>%f", lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "HCal_clus_e_cut")),
			// Form("(bb.sh.e+bb.ps.e)>%f", lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "SH_PS_clus_e_cut") - lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "SH_PS_sigma"))

			///THIS IS FOR THE COMPARISON BETWEEN PARSED AND NOT-PARSED:
			...
			// // Form("((abs(((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))))-%f)<%f", lookup_cut(runnum, "Ep"), lookup_cut(runnum, "Ep_sigma")),
			// Form("sbs.hcal.ed>%f", lookup_parsed_cut(run_target.Data(), kine, sbsfieldscale, "HCal_min")),
			// Form("(bb.sh.e+bb.ps.e)>%f", lookup_parsed_cut(run_target.Data(), kine, sbsfieldscale, "SH_PS_min"))

			//11449:
			// Form("bb.ps.e>%f", 0.15),
			// Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))>(%f)&&((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))<(%f)", 0.7, 1.25),
			// Form("sbs.hcal.e>%f", 0.03),
			// Form("(bb.sh.e+bb.ps.e)>%f", 1.6)
		};
	...
	for(Long64_t nevent = 0; nevent < Nevents; nevent++){
		...
		// cout << "diff: " << diff << ", hcal_time: " << hcal_time << ", bbcal_time: " << bbcal_time << endl;
...
//end of events loop  
	...
	cout << "SBS Field: " << sbsfieldscale << "%" << endl;
	cout << "-----------------------------------" << endl;
	cout << "BB angle: " << lookup_BB_angle_by_kine(kine) << endl;
	cout << "SBS angle: " << lookup_SBS_angle_by_kine(kine) << endl;
	cout << "HCal angle: " << HCal_theta << endl;
	cout << "HCal distance: " << HCal_dist << endl;
	cout << "-----------------------------------" << endl << endl;
	...
	cout << "Shower + Pre-Shower cut: "<< lookup_MC_cut(kine, sbsfieldscale, "SH_PS_clus_e_cut") - lookup_MC_cut(kine, sbsfieldscale, "SH_PS_sigma") << endl;
	cout << "Vertex <= 0.075" << endl;
	cout << "Number of GEM planes hit > 3" << endl;
	cout << "Number of tracks per event = 1" << endl;
	cout << "Pre-shower cut: " << lookup_MC_cut(kine, sbsfieldscale, "PS_clus_e_cut") << endl;
	cout << "E/p: center = " << lookup_MC_cut(kine, sbsfieldscale, "Ep") << ", sigma = " << lookup_MC_cut(kine, sbsfieldscale, "Ep_sigma") << endl;
	cout << "HCal cluster energy: " << lookup_MC_cut(kine, sbsfieldscale, "HCal_clus_e_cut");
	cout << "------------------------------------------------------------------"<< endl;
	cout << "Cut lookups: " << endl;
	cout << "PS: " << lookup_MC_cut(kine, sbsfieldscale, "PS_clus_e_cut") << endl;
	cout << "SH_PS: " << lookup_MC_cut(kine, sbsfieldscale, "SH_PS_clus_e_cut") << "; " << lookup_MC_cut(kine, sbsfieldscale, "SH_PS_sigma") << endl;
	cout << "HCal_clus_e: " << lookup_MC_cut(kine, sbsfieldscale, "HCal_clus_e_cut") << endl;
	cout << "Ep: " << lookup_MC_cut(kine, sbsfieldscale, "Ep") << "; " << lookup_MC_cut(kine, sbsfieldscale, "Ep_sigma") << endl;
	cout << "W2: " << lookup_cut_by_kine_and_mag(kine, sbsfieldscale, "W2") << "; " << lookup_MC_cut(kine, sbsfieldscale, "W2_sigma") << endl;
	cout << "W: " << lookup_MC_cut(kine, sbsfieldscale, "W") << "; " << lookup_MC_cut(kine, sbsfieldscale, "W_sigma") << endl;
	cout << "------------------------------------------------------------------"<< endl << endl;
