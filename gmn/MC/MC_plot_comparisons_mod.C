...
// //Normalization
	...
	plot_cut_type = "wcut_fcut"; 

	if( histo_scaling == 0 ){    //	0 -> Integral to 1
		h_dx_MC->Scale(1/(h_dx_MC->Integral()));
		if( plot_cut_type == "none" ){
			h_dx_data->Scale(1/(h_dx_data->Integral()));
			dx_scale = (h_dx_data->GetMaximum()/h_dx_MC->GetMaximum());
		}
		if( plot_cut_type == "wcut" ){
			h_dx_wcut_data->Scale(1/(h_dx_wcut_data->Integral()));
			dx_scale = (h_dx_wcut_data->GetMaximum()/h_dx_MC->GetMaximum());		
		}
		if( plot_cut_type == "fcut" ){
			h_dx_fcut_data->Scale(1/(h_dx_fcut_data->Integral()));
			dx_scale = (h_dx_fcut_data->GetMaximum()/h_dx_MC->GetMaximum());		
		}
		if( plot_cut_type == "wcut_fcut" ){
			h_dx_wcut_fcut_data->Scale(1/(h_dx_wcut_fcut_data->Integral()));
