...
TEllipse *ellipse_TGraph = new TEllipse(0.25,0.25,.1,.2);
...
//
ROOT::Math::Minimizer *ellipse_TGraph_minimize(TH2D *g)
{
  if (!g) return 0; // just a precaution
  if (g->GetNbinsX() < 6) return 0; // just a precaution
...
void fitEllipseTGraphRMM(TH2D *g = ((TH2D *)0))
{
  if (!g) g = static_cast<TH2D*>(TF->Get("h_dxdy")); // create a "random" ellipse
