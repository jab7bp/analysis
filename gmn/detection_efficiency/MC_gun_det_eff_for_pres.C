...
int calc_method = 3; //1 isn't good right now
...
double sbs4_p_min = 2.26;
double sbs4_p_max = 2.51;

double p_central_sbs7 = 6.20;
double sbs7_p_min = 2.26;
double sbs7_p_max = 2.51;

double p_central_sbs8 = 3.22;
double sbs8_p_min = 2.26;
double sbs8_p_max = 2.51;

double p_central_sbs9 = 3.21;
double sbs9_p_min = 2.26;
double sbs9_p_max = 2.51;

double p_central_sbs11 = 8.13;
double sbs11_p_min = 2.26;
double sbs11_p_max = 2.51;

double p_central_sbs14 = 4.84;
double sbs14_p_min = 2.26;
double sbs14_p_max = 2.51;

double p_central_kine = 0.0;
int p_central_sbs4_bin, p_central_sbs7_bin, p_central_sbs8_bin, p_central_sbs9_bin, p_central_sbs11_bin, p_central_sbs14_bin;
...
vector<double> ep_sbs4_neut, ep_sbs4_prot;
vector<double> pN_sbs4_neut, pN_sbs4_prot;
vector<double> p_sbs4_neut, p_sbs4_prot, p_sbs7_neut, p_sbs7_prot, p_sbs8_neut, p_sbs8_prot;
vector<double> p_sbs9_neut, p_sbs9_prot, p_sbs11_neut, p_sbs11_prot, p_sbs14_neut, p_sbs14_prot;
...
TCanvas *c_HCalE_pBins_overlay_n[4];
TCanvas *c_HCalE_pBins_cut_n[4];

TCanvas *c_HCalE_pBins_overlay_p[4];
TCanvas *c_HCalE_pBins_cut_p[4];

vector<int> HCalE_pBins_select = {0, 19, 39, 79};
vector<double> HCalE_pBins_maxRange_x = {0.25, 1.2, 1.8, 1.8};
...
//MC
Double_t mc_ep, mc_epx, mc_epy, mc_epz, mc_vx, mc_vy, mc_vz, mc_nucl, mc_posx, mc_posy;
Double_t mc_npy, mc_np;

//Scattered Kinematics
double nu_n = 0.0, nu_p = 0.0, p_center_neut = 0.0, p_center_prot = 0.0;
double p_scat_proton = 0.0, p_scat_neutron = 0.0;
double p_Nucleon, E_beam;
double e_prime_theta;
...
TH1D *h_HCalE_array_below_thresh_n[pN_bins];

TH1D *h_HCalE_array_p[pN_bins];
TH1D *h_HCalE_array_cut_p[pN_bins];
TH1D *h_HCalE_array_below_thresh_p[pN_bins];
...
void MC_gun_det_eff_for_pres(){
	...
	if( Nucleon_Select == "proton" || Nucleon_Select == "both" ){
		...
		h2_HCal_pN_cut_p = new TH2D("h2_HCal_pN_cut_p", Form("Proton momentum with threshold = HCal E_{mean} / (4.0 each bin) - SBS%i; Nucleon Momentum (GeV/c); HCal Cluster Energy (GeV)", kine), 80, 1, 9, HCal_bins, HCal_min, HCal_max);

		tprof_h2_HCalE_pN_p = new TProfile("tprof_h2_HCalE_pN_p", Form("Proton Momentum Energy Profile - SBS%i; Nucleon Momentum (GeV/c); HCal Cluster Energy (GeV)", kine), pN_bins, pN_min, pN_max, HCal_min, HCal_max);
	}
	...
	if( Nucleon_Select == "neutron" || Nucleon_Select == "both" ){
		...
		h_det_eff_n = new TH1D("h_det_eff_n", Form("Neutron Detection Efficiency - SBS%i; Nucleon Momentum (GeV/c); Efficiency (%%)", kine), 80, 1, 9);
		...
		h2_HCal_pN_cut_n = new TH2D("h2_HCal_pN_cut_n", Form("Neutron momentum with threshold = HCal E_{mean} / (4.0 each bin) - SBS%i; Nucleon Momentum (GeV/c); HCal Cluster Energy (GeV)", kine), 80, 1, 9, HCal_bins, HCal_min, HCal_max);
	...
	if( Nucleon_Select == "proton" || Nucleon_Select == "both" ){
		p_central_sbs4_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs4);
		p_central_sbs7_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs7);
		p_central_sbs8_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs8);
		p_central_sbs9_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs9);
		p_central_sbs11_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs11);
		p_central_sbs14_bin = h_det_eff_p->GetXaxis()->FindBin(p_central_sbs14);
	}
	if( Nucleon_Select == "neutron" ){
		p_central_sbs4_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs4);
		p_central_sbs7_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs7);
		p_central_sbs8_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs8);
		p_central_sbs9_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs9);
		p_central_sbs11_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs11);
		p_central_sbs14_bin = h_det_eff_n->GetXaxis()->FindBin(p_central_sbs14);
	}
	...
	for(int ibin = 0; ibin < pN_bins; ibin++ ){
		...
		if( Nucleon_Select == "neutron" || Nucleon_Select == "both" ){
			h_HCalE_array_n[ibin] = new TH1D(Form("h_HCalE_array_n_%i", ibin), Form("h_HCalE_array_pbins_n_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);
			h_HCalE_array_cut_n[ibin] = new TH1D(Form("h_HCalE_array_cut_n_%i", ibin), Form("h_HCalE_array_pbins_cut_n_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);
			h_HCalE_array_below_thresh_n[ibin] = new TH1D(Form("h_HCalE_array_below_thresh_n_%i", ibin), Form("h_HCalE_array_below_thresh_n_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);

			h_HCalE_array_below_thresh_n[ibin]->SetFillStyle(3001);
			h_HCalE_array_below_thresh_n[ibin]->SetFillColor(kRed);
		}

		if( Nucleon_Select == "proton" || Nucleon_Select == "both" ){
			h_HCalE_array_p[ ibin ] = new TH1D(Form("h_HCalE_array_p_%i", ibin), Form("h_HCalE_array_pbins_p_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);
			h_HCalE_array_cut_p[ ibin ] = new TH1D(Form("h_HCalE_array_cut_p_%i", ibin), Form("h_HCalE_array_pbins_cut_p_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);
			h_HCalE_array_below_thresh_p[ibin] = new TH1D(Form("h_HCalE_array_below_thresh_p_%i", ibin), Form("h_HCalE_array_below_thresh_p_%i; Energy Deposited (MeV)", ibin), pN_bins, 0.0, histo_max);

			h_HCalE_array_below_thresh_p[ibin]->SetFillStyle(3001);
			h_HCalE_array_below_thresh_p[ibin]->SetFillColor(kRed);
		}
...
//--------------------------------------------------
		...
		TC->SetBranchStatus( "MC.mc_nucl", 1);
		TC->SetBranchStatus( "MC.mc_npy", 1);
		TC->SetBranchStatus( "MC.mc_epz", 1);
		TC->SetBranchStatus( "MC.mc_np", 1);

		// MC Tree Variables
		TC->SetBranchAddress( "MC.mc_ep", &mc_ep);
		TC->SetBranchAddress( "MC.mc_epx", &mc_epx);
		TC->SetBranchAddress( "MC.mc_epy", &mc_epy);
		TC->SetBranchAddress( "MC.mc_epz", &mc_epz);
		...
		TC->SetBranchAddress( "MC.mc_npy", &mc_npy);
		TC->SetBranchAddress( "MC.mc_epz", &mc_epz);
		TC->SetBranchAddress( "MC.mc_np", &mc_np);
		...
		//Q-Vector
			double bb_track_p = sqrt( pow(mc_epx, 2.0) + pow(mc_epy, 2.0) + pow(mc_epz, 2.0) );
			TVector3 vertex(mc_vx, mc_vy, mc_vz); //Target Vertex vector in Hall Coordinates
			Double_t theta_nucleon_exp = acos( mc_epz/bb_track_p ); //Scattered Nucleon Theta
			Double_t phi_nucleon_exp = atan2( mc_epy, mc_epx ); //Scattered Nucleon Phi
			// Projected q vector:
		...
		//For momentume band calcs
			TLorentzVector P_beam( 0, 0, E_beam, E_beam ); //"mFinal" Beam energy
			TLorentzVector k_prime( mc_epx, mc_epy, mc_epz, bb_track_p );
			TLorentzVector P_targ( 0, 0, 0, Nucleon_mass);
			// e_prime_theta = acos( k_prime.Pz() / k_prime.E() );
			e_prime_theta = acos(mc_epz / bb_track_p);

			TLorentzVector q = P_beam - k_prime;
			pN = q + P_targ;

			pN_mag = sqrt( pow( pN[0], 2 ) + pow( pN[1], 2 ) + pow( pN[2], 2 ) );

			//anu method:
			TLorentzVector Pelectron(0,0,E_beam,E_beam);

			double E_prime = q.M2() / (2*E_beam*(1-cos(e_prime_theta)));
			TLorentzVector Pprimeelectron(E_prime*sin(e_prime_theta), 0, E_prime*cos(e_prime_theta), E_prime);
		...
		//Active area on HCal 
			...
			if( nucl_type == "neutron" ){
				...
				h2_HCal_pN_n->Fill(mc_ep, HCal_e);
				tprof_h2_HCalE_pN_n->Fill(mc_ep, HCal_e, 1.0);
				...
				int bin_i = int((mc_ep - pN_min)/bin_width );
				...
				//Momentum calcs:
				p_center_neut = E_beam/(1.0 + (E_beam/Mn)*( 1.0 - cos(e_prime_theta)));
				nu_n = P_beam.E() - bb_track_p;

				p_scat_neutron = sqrt( pow(nu_n, 2.0) + 2.0*(Mn*nu_n));
				p_Nucleon = p_scat_neutron;

				p_central = 0.0;
				p_central = (mc_ep - pN_min);

			//ANU'S METHOD
				TLorentzVector Pneutron(0,0,0, Mn);
				TLorentzVector Pprimeneutron;
				Pprimeneutron = P_beam + Pneutron - Pprimeelectron;
			///////////////////

				double neutron_3momentum_mag = Pprimeneutron.Vect().Mag();

				if( bin_i == p_central_sbs4_bin ){
					ep_sbs4_neut.push_back(mc_ep);
					p_sbs4_neut.push_back(p_scat_neutron);
					pN_sbs4_neut.push_back(pN_mag);
				}
				if( bin_i == p_central_sbs7_bin ){
					p_sbs7_neut.push_back(neutron_3momentum_mag);
				}
				if( bin_i == p_central_sbs8_bin ){
					p_sbs8_neut.push_back(p_scat_neutron);
					// cout << "-------------------" << endl;
					// cout << "sbs 8 neutron mc_ep: " << mc_ep << endl;
					// cout << "bin: " << bin_i << endl;
					// cout << "p_central: " << (mc_ep - pN_min) << endl;
					// cout << "p_scat_neutron: " << p_scat_neutron << endl;
					// // cout << "anu momentum: " << neutron_3momentum_mag << endl;
					// cout << "e_prime_theta: " << e_prime_theta << endl;
				}
				if( bin_i == p_central_sbs9_bin ){
					p_sbs9_neut.push_back(p_scat_neutron);
				}
				if( bin_i == p_central_sbs11_bin ){
					p_sbs11_neut.push_back(p_scat_neutron);
				}
				if( bin_i == p_central_sbs14_bin ){
					p_sbs14_neut.push_back(p_scat_neutron);
				}
			...
			if( nucl_type == "proton" ){
				...
				h2_HCal_pN_p->Fill(mc_ep, HCal_e);
				tprof_h2_HCalE_pN_p->Fill(mc_ep, HCal_e, 1.0);

				//Fill the histogram for various pN values
				// double bin_width = (pN_max - pN_min)/(pN_bins);
				double bin_width = h2_HCal_pN_n->GetXaxis()->GetBinWidth(1);
				int bin_i = (mc_ep - pN_min)/bin_width;
				...
				//Momentum calcs:
				p_center_prot = E_beam/(1.0 + (E_beam/Mp)*( 1.0 - cos(e_prime_theta)));
				nu_p = P_beam.E() - mc_ep;

				p_scat_proton = sqrt( pow(nu_p, 2.0) + 2.0*(Mp*nu_p));
				p_Nucleon = p_scat_proton;	

				p_central = 0.0;
				p_central = (mc_ep - pN_min);

			//ANU'S METHOD
				TLorentzVector Pproton(0,0,0, Mp);
				TLorentzVector Pprimeproton;
				Pprimeproton = P_beam + Pproton - Pprimeelectron;
			//////////////////////////////////
				double proton_3momentum_mag = Pprimeproton.Vect().Mag();

				if( bin_i == p_central_sbs4_bin ){
				// if( p_center_prot > (0.95)*p_central_sbs4 && p_center_prot < (1.05)*p_central_sbs4 ){
					ep_sbs4_prot.push_back(mc_ep);
					p_sbs4_prot.push_back(proton_3momentum_mag);
					pN_sbs4_prot.push_back(pN_mag);
				}
				if( bin_i == p_central_sbs7_bin ){
					p_sbs7_prot.push_back(p_central);
				}
				if( bin_i == p_central_sbs8_bin ){
					p_sbs8_prot.push_back(p_scat_proton);
				}
				if( bin_i == p_central_sbs9_bin ){
					p_sbs9_prot.push_back(p_scat_proton);
				}
				if( bin_i == p_central_sbs11_bin ){
					p_sbs11_prot.push_back(p_scat_proton);
				}
				if( bin_i == p_central_sbs14_bin ){
					p_sbs14_prot.push_back(p_scat_proton);
				}		
			}	
	////////////////////////////
	//END OF WHILE LOOP FOR THRESH CALCS
	////////////////////////////
		}
...
//After that first loop we now need to determine bin-wise threshold (E_peak/4.0);
	...
	//Kinematics Again
		//Q-Vector
			TVector3 vertex(mc_vx, mc_vy, mc_vz); //Target Vertex vector in Hall Coordinates
			Double_t theta_nucleon_exp = acos( mc_epz/mc_ep ); //Scattered Nucleon Theta
			Double_t phi_nucleon_exp = atan2( mc_epy, mc_epx ); //Scattered Nucleon Phi
			// Projected q vector:
			TVector3 pN_hat( sin(theta_nucleon_exp)*cos(phi_nucleon_exp), sin(theta_nucleon_exp)*sin(phi_nucleon_exp), cos(theta_nucleon_exp) );
		...
		//Active area on HCal 
			...
			//Fill histograms for various nucleon momentum values.....
				// double bin_width = (pN_max - pN_min)/pN_bins;
				double bin_width = h2_HCal_pN_n->GetXaxis()->GetBinWidth(1);
				int bin = (mc_ep - pN_min)/bin_width;

				//Fill anti-threshold histo for presentation:
				if( HCal_e < threshold_n[bin] ){
					h_HCalE_array_below_thresh_n[bin]->Fill(HCal_e);
				}
				...
				// if( mc_ep > ( floor(p_central_sbs4*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs4*10.0)/10.0 ) && p_scat_neutron > 0.0){
				// 	p_sbs4_neut.push_back(p_scat_neutron);
				// }
				// if( mc_ep > ( floor(p_central_sbs7*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs7*10.0)/10.0 ) && p_scat_neutron > 0.0){
				// 	p_sbs7_neut.push_back(p_scat_neutron);
				// }
				// if( mc_ep > ( floor(p_central_sbs8*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs8*10.0)/10.0 ) && p_scat_neutron > 0.0){
				// 	p_sbs8_neut.push_back(p_scat_neutron);
				// }
				// if( mc_ep > ( floor(p_central_sbs9*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs9*10.0)/10.0 ) && p_scat_neutron > 0.0){
				// 	p_sbs9_neut.push_back(p_scat_neutron);
				// }
				// if( mc_ep > ( floor(p_central_sbs14*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs14*10.0)/10.0 ) && p_scat_neutron > 0.0){
				// 	p_sbs14_neut.push_back(p_scat_neutron);
				// }
			...
			//Fill histograms for various nucleon momentum values.....
				...
				if( HCal_e < threshold_p[bin] ){
					h_HCalE_array_below_thresh_p[bin]->Fill(HCal_e);
				}
				...
				// if( mc_ep > ( floor(p_central_sbs4*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs4*10.0)/10.0 ) && p_scat_proton > 0.0){
				// 	p_sbs4_prot.push_back(p_scat_proton);
				// }
				// if( mc_ep > ( floor(p_central_sbs7*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs7*10.0)/10.0 ) && p_scat_proton > 0.0){
				// 	p_sbs7_prot.push_back(p_scat_proton);
				// }
				// if( mc_ep > ( floor(p_central_sbs8*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs8*10.0)/10.0 ) && p_scat_proton > 0.0){
				// 	p_sbs8_prot.push_back(p_scat_proton);
				// }
				// if( mc_ep > ( floor(p_central_sbs9*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs9*10.0)/10.0 ) && p_scat_proton > 0.0){
				// 	p_sbs9_prot.push_back(p_scat_proton);
				// }
				// if( mc_ep > ( floor(p_central_sbs14*10.0)/10.0 ) && mc_ep < ( ceil(p_central_sbs14*10.0)/10.0 ) && p_scat_proton > 0.0){
				// 	p_sbs14_prot.push_back(p_scat_proton);
				// }					
			}
	///////////////////////////
	//END OF WHILE LOOP FOR APPLYING THRESHOLDS
	///////////////////////////
		}
		...
		//Labels for efficiency values	
			...
			tpt_det_eff_bins = new TPaveText(6.4, 81, 7.575, 93);
			tpt_det_eff_bins->SetFillColor(kCyan);
			tpt_det_eff_bins->SetBorderSize(1);

			tpt_det_eff_bins->AddText("Efficiencies from Nearest Bin");
		...
		//-----------------------------------
			...
			tpt_det_eff_fits = new TPaveText(7.625, 81, 8.8, 93);
...
//END of NUCLEONS loop
	...
	for( size_t pBin_sel = 0; pBin_sel < HCalE_pBins_select.size(); pBin_sel++ ){
		if( true ){
			if( true ){
				c_HCalE_pBins_overlay_n[pBin_sel] = new TCanvas(Form("c_HCalE_pBins_overlay_n_%zu", pBin_sel), Form("c_HCalE_pBins_overlay_neutron_%zu", pBin_sel), 600, 500);
				h_HCalE_array_n[HCalE_pBins_select[pBin_sel]]->GetXaxis()->SetRangeUser(0, HCalE_pBins_maxRange_x[pBin_sel] );
				h_HCalE_array_n[HCalE_pBins_select[pBin_sel]]->SetStats(false);
				h_HCalE_array_n[HCalE_pBins_select[pBin_sel]]->Draw();
				
				h_HCalE_array_below_thresh_n[HCalE_pBins_select[pBin_sel]]->SetStats(false);
				h_HCalE_array_below_thresh_n[HCalE_pBins_select[pBin_sel]]->Draw("same");
			}
	
			if( false ){
				c_HCalE_pBins_cut_n[pBin_sel] = new TCanvas(Form("c_HCalE_pBins_cut_n_%zu", pBin_sel), Form("c_HCalE_pBins_cut_neutron_%zu", pBin_sel), 600, 500);
				h_HCalE_array_cut_n[HCalE_pBins_select[pBin_sel]]->GetXaxis()->SetRangeUser(0, HCalE_pBins_maxRange_x[pBin_sel] );
				
				h_HCalE_array_cut_n[HCalE_pBins_select[pBin_sel]]->SetStats(false);
				h_HCalE_array_cut_n[HCalE_pBins_select[pBin_sel]]->Draw();				
			}

		}

		if( false ){
			c_HCalE_pBins_overlay_p[pBin_sel] = new TCanvas(Form("c_HCalE_pBins_overlay_p_%zu", pBin_sel), Form("c_HCalE_pBins_overlay_proton_%zu", pBin_sel), 600, 500);
			h_HCalE_array_p[HCalE_pBins_select[pBin_sel]]->GetXaxis()->SetRangeUser(0, HCalE_pBins_maxRange_x[pBin_sel] );
			
			h_HCalE_array_p[HCalE_pBins_select[pBin_sel]]->SetStats(false);
			h_HCalE_array_p[HCalE_pBins_select[pBin_sel]]->Draw();
			
			h_HCalE_array_below_thresh_p[HCalE_pBins_select[pBin_sel]]->SetStats(false);
			h_HCalE_array_below_thresh_p[HCalE_pBins_select[pBin_sel]]->Draw("same");	

			c_HCalE_pBins_cut_p[pBin_sel] = new TCanvas(Form("c_HCalE_pBins_cut_p_%zu", pBin_sel), Form("c_HCalE_pBins_cut_proton_%zu", pBin_sel), 600, 500);
			h_HCalE_array_cut_p[HCalE_pBins_select[pBin_sel]]->GetXaxis()->SetRangeUser(0, HCalE_pBins_maxRange_x[pBin_sel] );
			
			h_HCalE_array_cut_p[HCalE_pBins_select[pBin_sel]]->SetStats(false);
			h_HCalE_array_cut_p[HCalE_pBins_select[pBin_sel]]->Draw();
		}
