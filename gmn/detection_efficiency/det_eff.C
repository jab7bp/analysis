...
double dy_sig_multiplier = 3.0;
double W2_sig_multiplier = 3.0;
...
double dx_min_x = -0.5;
double dx_max_x = 0.5;
int dx_nbins = int(dx_bin_factor*(dx_max_x - dx_min_x));

double dy_min_y = -0.4;
double dy_max_y = 0.4;
int dy_nbins = int(dx_bin_factor*(dy_max_y - dy_min_y));
...
double reject_min = 0.35;
double reject_max = 1.16;
...
TF1 *tf1_W2_fullFit, *tf1_W2_gaus, *tf1_W2_bg_pol4, *tf1_W2_bg_pol4_MOD, *tf1_W2_bg_reject;

//W2 pol8 fit to Bg subtracted W2
TF1 *tf1_W2_bgSub_polN, *tf1_W2_bgSub_fullFit, *tf1_W2_bgSub_gaus;

//W2 fullFit (standard) fits
TH1 *h_tf1_W2_gaus, *h_tf1_W2_bg_pol4;
TH1D *h_W2_gaus, *h_W2_bg_pol4, *h_W2_bg_pol8;
TH1D *h_W2_bg_from_sub, *h_W2_sig_from_sub, *h_W2_sig_from_sub_MOD;
...
double W2_bgSub_gaus_integral, W2_bgSub_pol8_integral, W2_bgSub_fullFit_integral;
double dx_fullFit_integral, dx_pol_integral, dx_gaus_integral;

//Errors for fits and propagation
double W2_bgSub_fullFit_integral_error, W2_bgSub_gaus_integral_error, W2_bgSub_pol8_integral_error;
...
double W2_bgSub_gaus_min_x, W2_bgSub_gaus_max_x, W2_bgSub_gaus_min_bin, W2_bgSub_gaus_max_bin;
...
// --------------------------------------
	...
	//For detection efficiency we should have some general cuts on 
			...
			if( hit_on_HCal && SH_nclus>0 && PS_nclus>0 && (abs(bb_tr_vz[0])<=0.075) && (bb_ps_e>0.15) && (bb_nhits[0]>2) ){
				h_W2->Fill( W2 );	
			}
...
//-----------------------------------------------
	...
	if( fit_only ){
		...
		h_W2_sig_from_sub_MOD = new TH1D("h_W2_sig_from_sub_MOD", "h_W2_sig_from_sub_MOD", W2_nbins, W2_min_x, W2_max_x);
...
//----------------------------------------------------------
		...
		h_W2_in->Fit("tf1_W2_bg_reject", "RMS0+");
		...
		h_W2_in->Fit("tf1_W2_fullFit", "RMS0+");
	...
	//------ W2 BG Pol4
		...
		tf1_W2_bg_pol4 = new TF1("tf1_W2_bg_pol4", fitPol4, W2_min_x, W2_max_x, polN+1);
		tf1_W2_bg_pol4->SetNpx(W2_nbins);
		tf1_W2_bg_pol4->SetLineColor(9);
		tf1_W2_bg_pol4->SetLineStyle(7);
		for(int param = 0; param <= polN; param++){
			tf1_W2_bg_pol4->FixParameter(param, W2_fullFit_par[param + 3]);
		}
		...
		h_tf1_W2_bg_pol4->Scale(h_tf1_W2_bg_pol4->GetMaximum()/h_tf1_W2_bg_pol4->GetMaximum());

		h_W2_bg_pol4 = (TH1D*)h_tf1_W2_bg_pol4->Clone("h_W2_bg_pol4");

		// tf1_W2_bg_pol4->Draw("hist+same");

	//------ W2 Gaus

		tf1_W2_gaus = new TF1("tf1_W2_gaus", "gaus", W2_min_x, W2_max_x);
		tf1_W2_gaus->SetNpx(W2_nbins);
		tf1_W2_gaus->SetLineColor(6);
		tf1_W2_gaus->SetLineStyle(6);
		for(int param = 0; param < 3; param++ ){
			tf1_W2_gaus->FixParameter(param, W2_fullFit_par[param]);
		}

		h_tf1_W2_gaus = (TH1*)tf1_W2_gaus->GetHistogram();
		h_tf1_W2_gaus->Scale(1.0/h_tf1_W2_gaus->Integral(), "width");
		h_tf1_W2_gaus->Scale(1.0/h_tf1_W2_gaus->Integral(), "height");
		h_tf1_W2_gaus->Scale(tf1_W2_gaus->GetMaximum()/h_tf1_W2_gaus->GetMaximum());

		h_W2_gaus = (TH1D*)h_tf1_W2_gaus->Clone("h_W2_gaus");

		tf1_W2_gaus->Draw("hist+same");

	///---- Get "BG" histogram by subtracting SIG histogram from full h_W2_in histogram
		TCanvas *c_W2_bg_sub = new TCanvas("c_W2_bg_sub", "c_W2_bg_sub", 600, 500);
		h_W2_in->Draw();
		for(int bin = 0; bin <= W2_nbins; bin++){
			double sub_val = h_W2_in->GetBinContent(bin) - h_W2_gaus->GetBinContent(bin);
			h_W2_bg_from_sub->SetBinContent(bin, sub_val);
		}
		h_W2_bg_from_sub->SetLineColor(7);
		// h_W2_bg_from_sub->Draw("same");

	//--- Now Fit a pol8 to this background to get a better total fit:
		int W2_bgSub_polN_N = 4;
		if( true ){	
			tf1_W2_bgSub_polN = new TF1("tf1_W2_bgSub_polN", Form("pol%i", W2_bgSub_polN_N), W2_min_x, W2_max_x);
			tf1_W2_bgSub_polN->SetNpx(W2_nbins);
			tf1_W2_bgSub_polN->SetLineColor(9);
			tf1_W2_bgSub_polN->SetLineStyle(9);
			for(int param = 0; param < W2_bgSub_polN_N; param++ ){
				tf1_W2_bgSub_polN->SetParName(param, Form("W2_bgSub_pol%i p%i", W2_bgSub_polN_N, param));
			}
		}
		if( false ){	
			tf1_W2_bgSub_polN = new TF1("tf1_W2_bgSub_polN", fitPol4_with_reject, 0.0, 1.4, 5);
			tf1_W2_bgSub_polN->SetNpx(W2_nbins);
			tf1_W2_bgSub_polN->SetLineColor(9);
			tf1_W2_bgSub_polN->SetLineStyle(9);
		}
		...
		for(int param = 0; param <= polN; param++){
			tf1_W2_bgSub_polN->FixParameter(param, W2_fullFit_par[param + 3]);
		}
		if( false ){
			h_W2_bg_from_sub->Fit("tf1_W2_bgSub_polN", "RMS+");		
		}
		if( true ){
			h_W2_in->Fit("tf1_W2_bgSub_polN", "RMS");
		}

		W2_bgSub_pol8_integral_error = tf1_W2_bgSub_polN->IntegralError(W2_min_x, W2_max_x)/tf1_W2_bgSub_polN->GetHistogram()->GetBinWidth(1);

		h_tf1_W2_bgSub_polN = (TH1*)tf1_W2_bgSub_polN->GetHistogram();
		h_tf1_W2_bgSub_polN->Scale(1.0/h_tf1_W2_bgSub_polN->Integral(), "width");
		h_tf1_W2_bgSub_polN->Scale(1.0/h_tf1_W2_bgSub_polN->Integral(), "height");
		h_tf1_W2_bgSub_polN->Scale(tf1_W2_bgSub_polN->GetMaximum()/h_tf1_W2_bgSub_polN->GetMaximum());

		h_W2_bgSub_polN = (TH1D*)h_tf1_W2_bgSub_polN->Clone("h_W2_bgSub_polN");

		tf1_W2_bgSub_polN->GetParameters(W2_bgSub_polN_par);

	//----Now we re-do the full fit (fullBGCutFitFunction) but we fix the pol8 parameters.
		tf1_W2_bgSub_fullFit = new TF1("tf1_W2_bgSub_fullFit", fullBGSubFitFunction, W2_min_x, W2_max_x, W2_bgSub_polN_N+4);
		tf1_W2_bgSub_fullFit->SetNpx(W2_nbins);

		tf1_W2_bgSub_fullFit->SetParName(0, "W2_bgSub_fullFit Gaus Norm");
		tf1_W2_bgSub_fullFit->SetParName(1, "W2_bgSub_fullFit Gaus Mean");
		tf1_W2_bgSub_fullFit->SetParName(2, "W2_bgSub_fullFit Gaus Sigma");

		tf1_W2_bgSub_fullFit->SetParLimits(0, 0, h_W2_in->GetMaximum());
		tf1_W2_bgSub_fullFit->SetParLimits(1, 0.775, 0.9);
		tf1_W2_bgSub_fullFit->SetParLimits(2, 0.0, 0.2);

		for( int param = 0; param <= W2_bgSub_polN_N; param++){
			tf1_W2_bgSub_fullFit->SetParName(param + 3, Form("W2_bgSub_fullFit pol%i p%i", W2_bgSub_polN_N, param) );
			tf1_W2_bgSub_fullFit->FixParameter(param + 3, W2_bgSub_polN_par[param]);
		}
		tf1_W2_bgSub_fullFit->SetLineColor(2);
		h_W2_in->Fit("tf1_W2_bgSub_fullFit", "RMS+");
		c_W2_bg_sub->cd();
		tf1_W2_bgSub_polN->Draw("hist+same");
		tf1_W2_bgSub_fullFit->GetParameters(par);

		W2_bgSub_fullFit_integral_error = tf1_W2_bgSub_fullFit->IntegralError(W2_min_x, W2_max_x)/tf1_W2_bgSub_fullFit->GetHistogram()->GetBinWidth(1);

		double W2_bgSub_gaus_norm = tf1_W2_bgSub_fullFit->GetParameter(0);
		double W2_bgSub_gaus_mean = tf1_W2_bgSub_fullFit->GetParameter(1);
		double W2_bgSub_gaus_sigma = tf1_W2_bgSub_fullFit->GetParameter(2);

		tf1_W2_bgSub_gaus = new TF1("tf1_W2_bgSub_gaus", "gaus", W2_min_x, W2_max_x);
		tf1_W2_bgSub_gaus->SetNpx(W2_nbins);
		for( int param = 0; param < 3; param++ ){
			tf1_W2_bgSub_gaus->FixParameter(param, par[param]);
		}

	//Now lets get the "signal" Gaussian by subracting the pol8 bgSub histogram from h_W2_in
	//We base this off of the gaussian determined from the full fit 
		W2_bgSub_gaus_min_x = W2_bgSub_gaus_mean - W2_sig_multiplier*fabs(W2_bgSub_gaus_sigma);
		W2_bgSub_gaus_max_x = W2_bgSub_gaus_mean + W2_sig_multiplier*fabs(W2_bgSub_gaus_sigma);

		W2_bgSub_gaus_min_bin = h_W2_in->GetXaxis()->FindBin(W2_bgSub_gaus_min_x);
		W2_bgSub_gaus_max_bin = h_W2_in->GetXaxis()->FindBin(W2_bgSub_gaus_max_x);

		cout << endl << "W2_bgSub_gaus --- min x: " << W2_bgSub_gaus_min_x << ", max x: " << W2_bgSub_gaus_max_x << endl;
		cout << "W2_bgSub_gaus --- min bin: " << W2_bgSub_gaus_min_bin << ", max bin: " << W2_bgSub_gaus_max_bin << endl << endl;

		tf1_W2_bg_pol4_MOD = new TF1("tf1_W2_bg_pol4_MOD", "pol4", 0.0, 1.4);
		tf1_W2_bg_pol4_MOD->SetNpx(280);
		// tf1_W2_bg_pol4_MOD->FixParameter(0, 105);
		// tf1_W2_bg_pol4_MOD->FixParameter(0, 110);
		// tf1_W2_bg_pol4_MOD->FixParameter(0, 68);
		// tf1_W2_bg_pol4_MOD->FixParameter(0, 49);
		tf1_W2_bg_pol4_MOD->FixParameter(0, tf1_W2_bgSub_fullFit->GetParameter(3) );

		// tf1_W2_bg_pol4_MOD->FixParameter(1, -944);
		// tf1_W2_bg_pol4_MOD->FixParameter(1, -912.666);
		// tf1_W2_bg_pol4_MOD->FixParameter(1, -120);
		// tf1_W2_bg_pol4_MOD->FixParameter(1, 168);
		tf1_W2_bg_pol4_MOD->FixParameter(1, tf1_W2_bgSub_fullFit->GetParameter(4) );

		// tf1_W2_bg_pol4_MOD->FixParameter(2, 5470);
		// tf1_W2_bg_pol4_MOD->FixParameter(2, 5460);
		// tf1_W2_bg_pol4_MOD->FixParameter(2, 2000);
		// tf1_W2_bg_pol4_MOD->FixParameter(2, 1405);
		tf1_W2_bg_pol4_MOD->FixParameter(2, tf1_W2_bgSub_fullFit->GetParameter(5) );

		// tf1_W2_bg_pol4_MOD->FixParameter(3, -7350);
		// tf1_W2_bg_pol4_MOD->FixParameter(3, -7355.07);
		// tf1_W2_bg_pol4_MOD->FixParameter(3, -3600);
		// tf1_W2_bg_pol4_MOD->FixParameter(3, -2865);
		tf1_W2_bg_pol4_MOD->FixParameter(3, tf1_W2_bgSub_fullFit->GetParameter(6) );

		// tf1_W2_bg_pol4_MOD->FixParameter(4, 3460);
		// tf1_W2_bg_pol4_MOD->FixParameter(4, 3470);
		// tf1_W2_bg_pol4_MOD->FixParameter(4, 2279);
		// tf1_W2_bg_pol4_MOD->FixParameter(4, 1932);
		tf1_W2_bg_pol4_MOD->FixParameter(4, tf1_W2_bgSub_fullFit->GetParameter(7) );

		tf1_W2_bg_pol4_MOD->SetLineColor(6);
		tf1_W2_bg_pol4_MOD->SetLineStyle(6);

		// h_W2_in->Fit("tf1_W2_bg_pol4_MOD", "R+");

		// TH1 *h_tf1_W2_bgSub_gaus_MOD = (TH1*)tf1_W2_bg_pol4_MOD->GetHistogram();
		// h_tf1_W2_bgSub_gaus_MOD->Scale(1.0/h_tf1_W2_bgSub_gaus_MOD->Integral(), "width");
		// h_tf1_W2_bgSub_gaus_MOD->Scale(1.0/h_tf1_W2_bgSub_gaus_MOD->Integral(), "height");
		// h_tf1_W2_bgSub_gaus_MOD->Scale(tf1_W2_bg_pol4_MOD->GetMaximum()/h_tf1_W2_bgSub_gaus_MOD->GetMaximum());

		// TH1D *h_W2_bg_pol4_MOD = (TH1D*)h_tf1_W2_bgSub_gaus_MOD->Clone("h_tf1_W2_bgSub_gaus_MOD");

		for( int bin = 0; bin <= W2_nbins; bin++ ){
			double sub_val = h_W2_in->GetBinContent(bin) - h_W2_bgSub_polN->GetBinContent(bin);

			// if( bin >= W2_bgSub_gaus_min_bin && bin <= W2_bgSub_gaus_max_bin ){
				h_W2_sig_from_sub->SetBinContent(bin, sub_val);
			// }
			// if( bin >= W2_bgSub_gaus_min_bin && bin <= W2_bgSub_gaus_max_bin ){
			// 	h_W2_sig_from_sub->SetBinContent(bin, h_W2_in->GetBinContent(bin) - h_W2_bg_pol4_MOD->GetBinContent(bin));
			// }

		}
		h_W2_sig_from_sub->SetLineColor(6);
		h_W2_sig_from_sub->Draw("hist+same");

		TLine *tl_W2_bgSub_sig_gaus_min = new TLine( W2_bgSub_gaus_min_x, 0, W2_bgSub_gaus_min_x, h_W2_in->GetMaximum() );
		tl_W2_bgSub_sig_gaus_min->SetLineStyle(7);
		tl_W2_bgSub_sig_gaus_min->SetLineColor(33);
		tl_W2_bgSub_sig_gaus_min->Draw("same");

		TLine *tl_W2_bgSub_sig_gaus_max = new TLine( W2_bgSub_gaus_max_x, 0, W2_bgSub_gaus_max_x, h_W2_in->GetMaximum() );
		tl_W2_bgSub_sig_gaus_max->SetLineStyle(7);
		tl_W2_bgSub_sig_gaus_max->SetLineColor(33);
		tl_W2_bgSub_sig_gaus_max->Draw("hist+same");


		h_W2_sig_from_sub->Fit("tf1_W2_bgSub_gaus", "RMS0+");
		W2_bgSub_gaus_integral_error = tf1_W2_bgSub_gaus->IntegralError(W2_min_x, W2_max_x)/tf1_W2_bgSub_gaus->GetHistogram()->GetBinWidth(1);

		W2_bgSub_fullFit_integral = tf1_W2_bgSub_fullFit->Integral(W2_min_x, W2_max_x)/tf1_W2_bgSub_fullFit->GetHistogram()->GetBinWidth(1);
		W2_bgSub_pol8_integral = tf1_W2_bgSub_polN->Integral(W2_min_x, W2_max_x)/tf1_W2_bgSub_polN->GetHistogram()->GetBinWidth(1);
		W2_bgSub_gaus_integral = h_W2_sig_from_sub->Integral();

		// W2_addsub_error = sqrt( pow( W2_bgSub_fullFit_integral_error, 2) + pow( W2_bgSub_pol8_integral_error, 2) );
		W2_addsub_error = sqrt( pow( W2_bgSub_fullFit_integral_error, 2) + pow( W2_bgSub_gaus_integral_error, 2) );

		TLegend *tl_W2_bg_sub = new TLegend(0.15, 0.7, 0.45, 0.85);
		tl_W2_bg_sub->AddEntry(tf1_W2_bgSub_fullFit, "Total W2 fit");
		tl_W2_bg_sub->AddEntry(h_W2_sig_from_sub, "W2 peak from BG sub.");
		tl_W2_bg_sub->AddEntry(tf1_W2_bgSub_polN, "BG fit");
		tl_W2_bg_sub->Draw("Same");

		TPaveText *tp_W2_bg_sub = new TPaveText(0.15, 0.60, 0.45, 0.69, "NDCbr");
		tp_W2_bg_sub->AddText(Form("W2 count = %i +/- %i", int(W2_bgSub_gaus_integral), int(W2_bgSub_gaus_integral_error) ));
		tp_W2_bg_sub->AddText(Form("Total fit #chi^{2} = %.1f", tf1_W2_bgSub_fullFit->GetChisquare() ));
		tp_W2_bg_sub->Draw("same");

		cout << endl << "-------------------------------" << endl;
		cout << "W2 bgSub fullFit ChiSquare: " << tf1_W2_bgSub_fullFit->GetChisquare() << endl;
		cout << "W2 gaus_min: " << W2_bgSub_gaus_min_x << endl;
		cout << "W2 gaus_max: " << W2_bgSub_gaus_max_x << endl << endl;
		cout << "W2 bgSub counts: " << endl;
		cout << "Full Integral: " << W2_bgSub_fullFit_integral << endl;
		cout << "BG integral: " << W2_bgSub_pol8_integral << endl;
		cout << "---" << endl;
		cout << "W2 bgSub gaus integral from function: " << gaus_integral(par[0], par[2], h_W2_in->GetBinWidth(1)) << endl;
		cout << "---" << endl;
		cout << "W2 signal after bgSub integral: " << W2_bgSub_gaus_integral << endl;
		cout << "W2 signal from sub error: " << W2_bgSub_gaus_integral_error << endl;
		cout << "-------------------------------" << endl << endl;
...
//----------------------------------------------------------
		...
		tf1_dy_fullFit->SetParLimits(1, -0.05, 0.5);
		tf1_dy_fullFit->SetParLimits(2, 0.0, 0.20);

		h_dy_in->Fit("tf1_dy_fullFit", "RMS+");
		...
		tf1_dy_gaus->SetNpx(dy_nbins);
		h_dy_in->Fit("tf1_dy_gaus", "RMS0+");
	...
	//----------------------------------------------------------
		...
		h_dx_in->Fit("tf1_dx_fullFit", "RMS+");
	...
	//We base this off of the gaussian determined from the full fit 
		dx_gaus_min_x = dx_gaus_mean - 4*fabs(dx_gaus_sigma);
		dx_gaus_max_x = dx_gaus_mean + 4*fabs(dx_gaus_sigma);
		...
		h_dx_sig_from_sub->Fit("tf1_dx_bgSub_gaus", "RMS0+");
	...
	/////	---------------------print stuff
		...
		det_eff_FINAL = 100.0*dx_gaus_integral/W2_bgSub_gaus_integral;

		//Only including error from ratio for efficiency  W2_bgSub_gaus_integral_error
		// det_eff_total_error = det_eff_FINAL*sqrt( pow( W2_bgSub_gaus_integral_error/W2_bgSub_gaus_integral, 2) + pow( dx_bgSub_gaus_integral_error/dx_gaus_integral, 2) );

		det_eff_total_error = det_eff_FINAL*sqrt( pow( (dx_addsub_error/dx_gaus_integral), 2) + pow( (W2_addsub_error/W2_bgSub_gaus_integral) , 2) );

		double sqrt_N_err = det_eff_FINAL*sqrt( pow( sqrt(dx_gaus_integral)/dx_gaus_integral, 2 ) + pow( sqrt(W2_bgSub_gaus_integral)/W2_bgSub_gaus_integral, 2) );
		...
		cout << "---" << endl;
		cout << "W2 signal integral: " << W2_bgSub_gaus_integral << " +/- " << W2_bgSub_gaus_integral_error << endl;
		cout << "-------------------------------" << endl;
		cout << "HCal Detector Efficiency = " << det_eff_FINAL << "% +/- " << det_eff_total_error << "% " << endl << endl;
		cout << "Error from sqrt(N_integrals) = " << sqrt_N_err << endl;
