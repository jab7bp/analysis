...
void xtalk_by_events(int runnum = 13770, int last_event_select = 5000, int segment = 0){
	...
	IN_DIR.Form("/lustre/expphy/volatile/halla/sbs/jboyd/Rootfiles/xtalk/%i", runnum);	
	OUT_DIR.Form("/lustre19/expphy/volatile/halla/sbs/jboyd/Rootfiles/xtalk/xtalk_by_events/%i", runnum);

	inputfile.Form("%s/e1209019_fullreplay_%i_*", IN_DIR.Data(), runnum);
